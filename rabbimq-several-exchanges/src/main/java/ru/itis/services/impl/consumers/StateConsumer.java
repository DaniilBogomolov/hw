...
@Component("gstaff_state")
public class StateConsumer extends AbstractConsumer {

    public static final String QUEUE_NAME = "state_staff_queue";
    public static final String PDF_TEMPLATE_NAME = "state.pdf";
    ...
    protected PdfDocument generateDocument() {
        return new PdfDocument(new PdfReader(PDF_TEMPLATE_NAME),
                new PdfWriter(new FileOutputStream("documents/" + new Date().toString() + UUID.randomUUID().toString() + ":state.pdf")));
    }
